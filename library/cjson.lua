...
---module. The new module has a separate persistent encoding buffer, and
---default settings.
---@return cjson
...
---crashing the application due to a lack of process stack space.
---
...
---The current setting is always returned, and is only updated when an argument
---is provided.
---@param depth? integer -- must be a positive integer.
...
---Available settings:
---
---- `true`: Allow *invalid numbers* to be encoded. This will generate
...
---
---- `false`: Throw an error when attempting to encode *invalid numbers*. This
---  is the default setting.
---
---The current setting is always returned, and is only updated when an argument
---is provided.
---@param setting? true | false | "null"
---@return true | false | "null" setting
...
---Available settings:
---
---- `true`: The buffer will grow to the largest size required and is not freed
---  until the Lua CJSON module is garbage collected. This is the default
---  setting.
---
...
---error. This prevents deeply nested or recursive data structure from crashing
---the application.
---
...
---containing many numbers, setting `cjson.encode_number_precision` to a
---smaller integer, for example `3`, can improve encoding performance by up to
---50%.
---
---By default, Lua CJSON will output 16 significant digits when converting a
---number to text.
---
---The current setting is always returned, and is only updated when an argument
---is provided.
...
---JSON array. This is determined by the number of values missing from the Lua
---array as follows:
---
---- **Normal**: All values are available.
---- **Sparse**: At least one value is missing.
---- **Excessively sparse**: The number of values missing exceeds the
---  configured ratio.
---
---Lua CJSON encodes sparse Lua arrays as JSON arrays using JSON `null` for the
---missing entries.
---
...
---@param ratio? integer -- must be a non-negative integer.
---@param safe? integer -- must be a positive integer.
---@return boolean convert, integer ratio, integer safe
...
---@param setting? true | false | "on" | "off"
---@return boolean setting
...
---array part of the table, and will be of the same lenght as the `#` operator
---on that table. Holes in the table will be encoded with the `null` JSON
---value.
---
---Example:
---
...
---(that is, if the table has no elements, `cjson.encode()` will encode it as
---an empty JSON Array).
---
---Instead of:
---
...
cjson.empty_array_mt = nil

---**default**: true
---
---If enabled, forward slash `/` will be encoded as `\/`
---
---If disabled, forward slash `/` will be encoded as `/` (no escape is
---applied).
...
---with the `array_mt` metatable. This can ensure 1-to-1 relationship between
