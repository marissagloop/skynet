...
function cluster.reload(config) end
---* 为某个节点上的服务生成一个代理服务 clusterproxy
---@param node string
---@param name string
function cluster.proxy(node, name) end
function cluster.snax(node, name, address) end
---* 可以把 addr 注册为 cluster 可见的一个字符串名字 name 。如果不传 addr 表示把自身注册为 name 。
---@param name string
---@param addr number
function cluster.register(name, addr) end
---* 查询节点上服务的地址
---@param node string
---@param name string
---@return number
function cluster.query(node, name) end
return cluster
