...
function sharemap.writer(typename, obj) end
---* writer 实际上是将一分 Lua 数据，经过 sproto 序列化后，重新放到内存中
function sharemap:commit() end
---* writer 使用，生成一个指针，用来传递到其他服务
function sharemap:copy() end
---* reader 使用，若stm对象有更新，将内存重新序列化出来。
function sharemap:update() end
---* 为一 stm 对象构造一个 reader
---@param typename any
---@param stmcpy any
---@return sharemapreader
function sharemap.reader(typename, stmcpy) end
